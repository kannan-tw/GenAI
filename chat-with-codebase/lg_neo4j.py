...
def simplifier(state):
    ...
    user_query = state['user_query'][-1] #"list functions from files"
...
def build_workflow():
    ...
    #workflow.add_edge("simplifier", "translate_to_cypher")
...
if __name__ == "__main__":
    ...
    thread = {"configurable": {"thread_id": 19}}
    ...
    while True:
        ...
        for output in graph.stream(
            {
                "user_query": [HumanMessage(content=user)],
            ...
            '''for key, value in output.items():
                print("Agent : ",key)
                print("value", value)
                #print("Result : ",value['sub_query'])
                if 'sub_query' in value and value['sub_query']:
                    content = value['sub_query']#.content
                    #print("Extracted Content:")
                    #print(content)
                    #print("*****\n")
                elif 'neo4j_schema' in value and value['neo4j_schema']:
                    #content = value['neo4j_schema'][-1] #.content
                    #print("Extracted Content:")
                    #print(content)
                    #print("*****\n")
                    pass
                elif 'user_query' in value and value['user_query']:
                    content = value['user_query'][-1].content
                    #print("Extracted Content:")
                    #print(content)
                    #print("*****\n")
                elif 'final_results' in value and value['final_results']:
                    #print(value)
                    #print(value['final_results'])
                    content = value['final_results'][-1]
                    #print("Extracted Content:")
                    #print(content)
                    #print("*****\n")
                else:
